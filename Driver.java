...
public class Driver {
    ...
    public static void main(String[] args) {
    	...
    	do {																	//do while to continue prompting until "x" is entered to quit
	    	try {																//try/catch block for exception handling
				displayMenu();													//call displayMenu() method
																			
				userChoice = scnr.nextLine();									//receive input from user based on displayMenu() options
				
				if (userChoice.equals("1")) {									//different methods are called based on user input
					addShip();
				}
				else if (userChoice.equals("2")) {								//switch statements could have also been used but I think if/else branches look cleaner
					editShip();
				}
			...
			} catch (Exception e) {														//catch block executes in the case of an exception
    ...
    public static void printShipList(String listType) {
        ...
        } else if (listType == "active") { 								// Print ships which contain 
